...
model = load_model('E:/INTERVIEW-VIVA-FRONT/your_model.keras')
...
def process():
    if request.method == 'POST':
        # Read job description text from the form
        rawtext = request.form['rawtext']
        jdtxt = [rawtext]

        # Read and preprocess resume text files from the UPLOAD_FOLDER
        resumetxt = []
        for filename in os.listdir(UPLOAD_FOLDER):
            if filename.endswith(".txt"):  # Ensure it reads only text files
                file_path = os.path.join(UPLOAD_FOLDER, filename)
                with open(file_path, 'r', encoding='utf-8') as file:
                    resumetxt.append(file.read())

        # Preprocess text from resumes and job description
        p_resumetxt = preprocess(resumetxt)
        p_jdtxt = preprocess(jdtxt)

        # Extract text features and reduce dimensions
        ...
        # Attach resume content to the dataframe
        ...
        # Extract skills based on predefined skill list
        ...
        # Sort by job description relevance
        sorted_dt = dt.sort_values(by=['JD 1'], ascending=False)

        # Save the result to CSV and return it as a download
...
class QuizApp:
    def __init__(self, root):
        ...
        self.emotion_model = load_model("E:/INTERVIEW-2/your_model.keras")
    ...
    def convert_audio_to_text(self, audio_file):
        ...
        try:
            ...
            filename_answer = os.path.join(r"E:/INTERVIEW-VIVA-FRONT/",
    ...
    ''' def save_result_to_csv(self, result_text):
        csv_filename = os.path.join("E:\INTERVIEW_PROJECT_working", "results.csv")
    ...
    def save_result_to_csv(self, result_text):
        csv_filename = os.path.join("E:/INTERVIEW-VIVA-FRONT/", "interview_results.csv")

        # Create new file if this is the first question (i.e., index == 1)
        write_header = not os.path.isfile(csv_filename) or self.current_question_index == 1

        try:
            with open(csv_filename, 'a' if not write_header else 'w', newline='', encoding='utf-8') as csv_file:
                csv_writer = csv.writer(csv_file)
                if write_header:
                    csv_writer.writerow(["Timestamp", "Question Number", "Similarity Result"])
                
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                csv_writer.writerow([timestamp, self.current_question_index, result_text])
...
def addrec():
    if request.method == 'POST':
        try:
            ...
            with sql.connect("mockemoji.db") as con:
                cur = con.cursor()
                cur.execute("INSERT INTO users(name,phono,email,username,password)VALUES(?, ?, ?, ?,?)",
...
def logindetails():
    if request.method == 'POST':
        usrname = request.form['username']
        passwd = request.form['password']
        print("Attempt login:", usrname)

        with sql.connect("mockemoji.db") as con:
            cur = con.cursor()
            cur.execute("SELECT username,password FROM users WHERE username=?", (usrname,))
            account = cur.fetchall()
            print("DB response:", account)
...
def predcrop():
    ...
    if request.method == 'POST':
        # Delete old CSV if exists before new interview
        csv_path = "E:/INTERVIEW-VIVA-FRONT/interview_results.csv"
        if os.path.exists(csv_path):
            os.remove(csv_path)

        # Start the interview UI
        root = tk.Tk()
        app = QuizApp(root)
        root.mainloop()
        ...
        # Extract similarity values and calculate average
        similarity_values = []
        import re
        for row in similarities:
            match = re.search(r"(\d+\.\d+)%", row["similarity"])
            if match:
                similarity_values.append(float(match.group(1)))

        avg_similarity = round(sum(similarity_values) / len(similarity_values), 2) if similarity_values else 0.0

        # Determine emotion feedback
        emotion_message = ""
        ...
        return render_template('result123.html',
                               max_emotion_label=app.max_emotion_label,
...
def get_last_three_similarities():
    with open("last_execution_results.csv", "r") as file:
        ...
        try:
            ...
            if len(parts) > 1:
                ...
                if match:
                    ...
                    print(f"Skipping malformed line (no percentage found): {line.strip()}")
...
def get_last_three_similarities():
    similarities = []
    file_path = "E:/INTERVIEW-VIVA-FRONT/interview_results.csv"
    ...
    lines = lines[1:] if lines and lines[0].startswith("Timestamp") else lines
    for line in lines[-3:]:
        parts = line.strip().split(",")
        if len(parts) > 2:
            match = re.search(r"(\d+\.\d+)%", parts[2])
            if match:
                similarities.append(float(match.group(1)))
    return similarities


@app.route('/result')
def result():
    similarities = get_last_three_similarities()
    print("Similarities read from CSV:", similarities)
    avg_similarity = round(sum(similarities) / len(similarities), 2) if similarities else 0.0
    print("Calculated average similarity:", avg_similarity)
    return render_template('result.html', avg_similarity=avg_similarity)

from viva import viva_route, questions



'''questions = [
    {"question": "What is the output of print(2 ** 3)?", "options": ["6", "8", "9", "5"], "answer": "8"},
    {"question": "Which keyword is used for function in Python?", "options": ["func", "def", "function", "define"], "answer": "def"},
    {"question": "Which of the following is a valid variable name?", "options": ["2var", "my-var", "_var", "my var"], "answer": "_var"},
    {"question": "What does len() function do?", "options": ["Returns length", "Returns index", "Returns type", "Returns range"], "answer": "Returns length"},
    {"question": "What is output of type(10)?", "options": ["int", "float", "<class 'int'>", "<type 'int'>"], "answer": "<class 'int'>"},
    {"question": "Which operator is used for floor division?", "options": ["/", "//", "%", "**"], "answer": "//"},
    {"question": "What is output of bool(0)?", "options": ["True", "False", "0", "None"], "answer": "False"},
    {"question": "Which one is a Python tuple?", "options": ["[1,2,3]", "{1,2,3}", "(1,2,3)", "set(1,2,3)"], "answer": "(1,2,3)"},
    {"question": "Which keyword is used for loop?", "options": ["loop", "iterate", "for", "goto"], "answer": "for"},
    {"question": "How do you start a comment?", "options": ["//", "#", "/*", "--"], "answer": "#"},
    {"question": "Which module is used for regular expressions?", "options": ["math", "sys", "re", "regex"], "answer": "re"},
    {"question": "What is the extension of Python file?", "options": [".txt", ".py", ".java", ".cpp"], "answer": ".py"},
    {"question": "Which function is used to get input?", "options": ["get()", "read()", "input()", "scan()"], "answer": "input()"},
    {"question": "What is output of 3 == '3'?", "options": ["True", "False", "Error", "None"], "answer": "False"},
    {"question": "What is the keyword to define class?", "options": ["function", "def", "object", "class"], "answer": "class"}
]'''

@app.route('/viva')
def viva_route():
    return render_template("viva.html", questions=questions)
...
def submit():
    ...
    return render_template("result2.html", score=score, total=total, answers=user_answers)
